...
public interface InventoryItemConfigServiceInterface {
     ...
     * @param itemId   the ID of the InventoryItem to modify
     * @param item     the modified InventoryItem to persist
     * @param updateStatus if true then update ARC item status created from this inventory item
     * @param updateDefaultPrice if true then update ARC item price created from this inventory item
     * @return the modified InventoryItem
     */
    @PUT
    @Path("{id}/v1")
    ...
    @Validated(InventoryItem.class)
    @PreAuthorize("hasPermission(\'Required\', \'WritePropertyConfig\')")
    InventoryItem updateInventoryItem(@PathParam("tenantId") String tenantId, @PathParam("propertyId") String propertyId,
          @PathParam("id") String itemId, @QueryParam("updateAutoRecurringDefaultPrice") boolean updateDefaultPrice,
          @QueryParam("updateAutoRecurringStatus") boolean updateStatus, InventoryItem item)
          throws RGuestException, ServiceException;
}
